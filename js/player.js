...
function loadPlayerInfo() {
    ...
    apiRequestWithProxy('action=').then(function(info) {
...
function loadCategories(type) {
    ...
    apiRequestWithProxy(action).then(function(cats) {
...
function loadCategoryContent(cat) {
    ...
    apiRequestWithProxy(action).then(function(streams) {
...
function renderChannels() {
    ...
    currentChannels.forEach(function(ch, idx) {
        ...
        el.addEventListener('click', function(e) {
            ...
            document.querySelectorAll('.ch-item').forEach(function(i) { i.classList.remove('active'); });
            el.classList.add('active');
            playItem(ch, chType);
        });
...
function stopNativeVod() {
    if (!nativeVodActive) return Promise.resolve();
    nativeVodActive = false;
    var nativeApi = window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.EmbedPlayer;
    var p = Promise.resolve();
    if (nativeApi) p = nativeApi.stop();
...
function _doPlayWithFallback(video, rawUrl, isLive, name, attempt) {
    ...
    var url = (attempt === 0) ? rawUrl : getLocalProxyUrl(rawUrl);
    ...
    video.onerror = function() {
        ...
        if (err) {
            if (err.code === 4) msg = 'Desteklenmeyen format';
...
function loadSeriesSeasons(ch) {
    ...
    apiRequestWithProxy('action=get_series_info&series_id=' + ch.series_id).then(function(data) {
...
function loadLogoProxy(img) {
    ...
    var proxyUrl = '/proxy?url=' + encodeURIComponent(originalUrl);
    img.src = proxyUrl;
}
...
function checkForUpdate() {
    ...
    fetch(VERSION_CHECK_URL + '?t=' + Date.now()).then(function(r) { return r.json(); }).then(function(data) {
...
function searchSubtitles(movieName, onDone) {
    ...
    langs.forEach(function(lang) {
        var opts = isAndroid() ? { headers: { 'User-Agent': 'TemporaryUserAgent' } } : {};
        fetch(baseUrl + lang, opts).then(function(r) {
...
function loadSubtitleFromUrl(url, name, onDone) {
    if (!url) return;
    var fetchUrl = isAndroid() ? url : '/api/subtitles/download?url=' + encodeURIComponent(url);
    var fetchOpts = isAndroid() ? { headers: { 'User-Agent': 'TemporaryUserAgent', 'Referer': 'https://www.opensubtitles.org/' } } : {};
